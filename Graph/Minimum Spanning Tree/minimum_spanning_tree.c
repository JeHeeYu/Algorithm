...
// 프림 알고리즘
void Prim(Graph* g, Vertex* startVertex, Graph* treeGrah);

// 크루스칼 알고리즘
void Kruskal(Graph* g, Graph* tree);
...
int main()
{
    ...
    Graph* primTree = CreateGraph();
    Graph* kruskalTree = CreateGraph();
    ...
    Prim(graph, B, primTree);
    PrintGraph(primTree);
    
    printf("Kruskal's Algorithm...\n");
    Kruskal(graph, kruskalTree);
    PrintGraph(kruskalTree);
    
    // 그래프 소멸
    DestroyGraph(primTree);
    DestroyGraph(kruskalTree);
    DestroyGraph(graph);
...
void Prim(Graph* g, Vertex* startVertex, Graph* treeGrah)
{
    ...
    for(i = 0; i < g->vertexCount; i++) {
        ...
        AddEdge(treeVertices[toIndex], CreateEdge(treeVertices[toIndex], treeVertices[fromIndex], weights[i]));
    }
...
void Kruskal(Graph* g, Graph* tree)
{
    ...
    while ( currentVertex != NULL ) {
        ...
        treeVertices[i] = CreateVertex(currentVertex->data);
        AddVertex(tree, treeVertices[i]);

        currentEdge = currentVertex->adjacencyList;
        
        while (currentEdge != NULL) {
            Node newNode = { currentEdge->weight, currentEdge };
            Enqueue(queue, newNode);
    ...
    while (!IsEmpty(queue)) {
        ...
        Dequeue( queue,&tempNode);
        currentEdge = (Edge*)tempNode.data;
        ...
        if (FindSet(vertexSet[fromIndex]) != FindSet(vertexSet[toIndex])) {
            AddEdge(treeVertices[fromIndex], CreateEdge(treeVertices[fromIndex], treeVertices[toIndex], currentEdge->weight));

            AddEdge( treeVertices[toIndex], CreateEdge(treeVertices[toIndex], treeVertices[fromIndex], currentEdge->weight));
